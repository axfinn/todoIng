...
### 修复
- 完成 backend-go 的所有 API 兼容性修复
- 修复认证接口的响应格式和 JWT payload 结构
- 添加邮箱验证码验证支持
- 修复任务接口直接返回数组或对象而不是包装响应
- 统一所有错误响应使用 msg 字段
- 修复 JWT token 解析路径 (user.id)
- 修复所有模型的 ID 字段使用 _id JSON tag
- 完整实现报告接口功能
...
- 统一 backend-go 中所有错误响应格式
  - 将 tasks.go 中所有 `gin.H{"error": "xxx"}` 统一替换为 `gin.H{"msg": "xxx"}`
  - 确保与前端错误处理逻辑保持一致
