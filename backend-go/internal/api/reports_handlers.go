...
func (d *ReportDeps) ListReports(w http.ResponseWriter, r *http.Request) {
    ...
    defer cur.Close(ctx)
    var reports []bson.M
    for cur.Next(ctx) { var m bson.M; if cur.Decode(&m)==nil { if id, ok := m["_id"].(primitive.ObjectID); ok { m["_id"] = id.Hex() }; reports = append(reports, m) } }
...
func (d *ReportDeps) GetReport(w http.ResponseWriter, r *http.Request) {
    uid := GetUserID(r); if uid=="" { JSON(w,401,map[string]string{"msg":"Unauthorized"}); return }
    id := mux.Vars(r)["id"]
    objID, err := primitive.ObjectIDFromHex(id); if err!=nil { JSON(w,404,map[string]string{"msg":"Report not found"}); return }
    ctx, cancel := context.WithTimeout(r.Context(), 10*time.Second); defer cancel()
    var rep bson.M
    if err := d.DB.Collection("reports").FindOne(ctx, bson.M{"_id": objID, "userId": uid}).Decode(&rep); err != nil { JSON(w,404,map[string]string{"msg":"Report not found"}); return }
    rep["_id"] = id
            ...
            if taskIDStr, ok := taskID.(string); ok {
                if taskObjID, err := primitive.ObjectIDFromHex(taskIDStr); err == nil {
                    ...
                    if err := d.DB.Collection("tasks").FindOne(ctx, bson.M{"_id": taskObjID}).Decode(&task); err == nil {
                        if id, ok := task["_id"].(primitive.ObjectID); ok { task["_id"] = id.Hex() }
...
func (d *ReportDeps) GenerateReport(w http.ResponseWriter, r *http.Request) {
    ...
    if req.Type=="" || req.Period=="" || req.StartDate=="" || req.EndDate=="" { JSON(w,400,map[string]string{"msg":"Please provide type, period, startDate, and endDate"}); return }
    if req.Type!="daily" && req.Type!="weekly" && req.Type!="monthly" { JSON(w,400,map[string]string{"msg":"Invalid type"}); return }
    
    // 支持多种日期格式，兼容前端
    start, err1 := parseFlexibleDate(req.StartDate)
    end, err2 := parseFlexibleDate(req.EndDate)
    if err1!=nil || err2!=nil { JSON(w,400,map[string]string{"msg":"Invalid date format"}); return }
    ...
    var tasks []bson.M
    for tasksCur.Next(ctx) { var t bson.M; if tasksCur.Decode(&t)==nil { if id, ok := t["_id"].(primitive.ObjectID); ok { t["_id"] = id.Hex() }; tasks = append(tasks, t) } }
    total := len(tasks)
    completed := 0
    inProgress := 0
    overdue := 0
    ...
    for _, t := range tasks {
        ...
        if status == "Done" { completed++ } else if status == "In Progress" { inProgress++ }
        if ddl, ok := t["deadline"].(time.Time); ok {
            if ddl.Before(now) && status != "Done" { overdue++ }
        }
    }
    completionRate := 0
    if total > 0 { completionRate = int(float64(completed) / float64(total) * 100) }
    ...
    if len(tasks)==0 {
        ...
        for _, t := range tasks {
            ...
            if !createdAt.IsZero() { sb.WriteString("- **创建时间**: "+createdAt.Format("2006-01-02 15:04:05")+"\n") }
            if !updatedAt.IsZero() { sb.WriteString("- **更新时间**: "+updatedAt.Format("2006-01-02 15:04:05")+"\n") }
            if ddl, ok := t["deadline"].(time.Time); ok && !ddl.IsZero() { sb.WriteString("- **截止日期**: "+ddl.Format("2006-01-02 15:04:05")+"\n") }
            if sch, ok := t["scheduledDate"].(time.Time); ok && !sch.IsZero() { sb.WriteString("- **计划日期**: "+sch.Format("2006-01-02 15:04:05")+"\n") }
            desc, _ := t["description"].(string)
            if desc=="" { desc = "无" }
            sb.WriteString("- **任务描述**: "+desc+"\n\n")
            sb.WriteString("#### 任务活动时间线\n")
            sb.WriteString("- "+createdAt.Format("2006-01-02 15:04:05")+": 任务已创建\n")
            if !updatedAt.IsZero() && updatedAt.After(createdAt.Add(5*time.Second)) { sb.WriteString("- "+updatedAt.Format("2006-01-02 15:04:05")+": 任务已更新\n") }
            sb.WriteString("\n---\n\n")
        }
    }
    content := sb.String()
...
func extractTaskIDs(tasks []bson.M) []string {
    ...
    for _, t := range tasks { if id, ok := t["_id"].(string); ok { ids = append(ids, id) } }
    return ids
}
...
func (d *ReportDeps) PolishReport(w http.ResponseWriter, r *http.Request) {
    ...
    polished := original
    if pc, ok := rep["polishedContent"].(string); ok && pc != "" { polished = pc } else { polished = original + "\n\n[Polished Placeholder]" }
...
func (d *ReportDeps) DeleteReport(w http.ResponseWriter, r *http.Request) {
    ...
    if err != nil || res.DeletedCount==0 { JSON(w,404,map[string]string{"msg":"Report not found"}); return }
...
func (d *ReportDeps) ExportReport(w http.ResponseWriter, r *http.Request) {
    ...
    if err := d.DB.Collection("reports").FindOne(ctx, bson.M{"_id": objID, "userId": uid}).Decode(&rep); err != nil { JSON(w,404,map[string]string{"msg":"Report not found"}); return }
    content := ""
    if pc, ok := rep["polishedContent"].(string); ok && pc != "" { content = pc } else { content = rep["content"].(string) }
    fileExt := "txt"
    contentType := "text/plain"
    if format == "md" || format == "markdown" { fileExt = "md"; contentType = "text/markdown" }
...
func itoa(i int) string {
    if i==0 { return "0" }
    b := make([]byte,0,6)
    for i>0 { b = append([]byte{byte('0'+ i%10)}, b...); i/=10 }
    return string(b)
}
